...
from ftp_op import *
from ftp_put import PutHandler
import io
import time, os, sys
import struct
import random
    ...
    def handle_list_reply(self, op, m):
        ...
        if op.opcode == OP_Ack:
            ...
            for d in dentries:
                ...
                if d[0] == "D":
                    # output += "D %s\n" % d[1:]
                    output.append({"name": d[1:], "is_dir": True, "size_b": 0})
                elif d[0] == "F":
                    (name, size) = d[1:].split("\t")
                    size = int(size)
                    self.total_size += size
                    output.append({"name": name, "size_b": size, "is_dir": False})
                else:
                    pass
            ...
            self.ftp.list_result = self.list_temp_result
            self.total_size = 0
...
class FTPModule:
    def __init__(self, mav):
        ...
        self.put2_result = None
        self.create_result = None
        self.open_wo_result = None
        self.list_result = None
    ...
    def cmd_get(self, args, callback=None, callback_progress=None):
        ...
        if len(args) == 0:
            ...
            return
        if len(args) == 4:
            self.requested_offset = args[2]
            self.requested_size = args[3]
        else:
        ...
        if callback is None or self.ftp_settings["debug"] > 1:
            ...
            print("offset: %s, size: %s" % (self.requested_offset, self.requested_size))
    ...
    def handle_open_RO_reply(self, op, m):
        ...
        if op.opcode == OP_Ack:
            ...
            read = FTP_OP(self.seq, self.session, OP_BurstReadFile, self.burst_size, 0, 0, self.requested_offset, None)
            print(read)
    ...
    def handle_open_WO_reply(self, op, m):
        """handle OP_OpenFileRO reply"""
        if op.opcode == OP_Ack:
            self.open_wo_result = True
    ...
    def check_read_finished(self):
        ...
        if len(self.read_gaps) == 0 and (self.reached_eof or self.read_total >= self.requested_size):
            ...
            with open(self.filename, "rb") as f:
                f.seek(self.requested_offset)
                self.get_result = f.read(self.requested_size)
            # print(self.get_result.decode("utf-8"))
            print(f"terminating with {self.read_total} out of {self.requested_size}")
            self.terminate_session()
            return True
        return False
    ...
    def cmd_open_wo(self, path):
        enc_fname = bytearray(path, "ascii")
        op = FTP_OP(self.seq, self.session, OP_OpenFileWO, len(enc_fname), 0, 0, 0, enc_fname)
        self.send(op)
    ...
    def cmd_put2(self, params):
        logger.info(f"FTP: cmd_put2 {params}")
        file, offset, data, fh = params
        return self.put_handler.send_data(file, offset, data, fh)
    ...
    def handle_create_file_reply(self, op, m):
        ...
        if self.fh is None and  not self.put_handler.in_progress:
            logger.info("terminating session1")
            self.terminate_session()
            return
        if op.opcode == OP_Ack:
    ...
    def send_more_writes(self):
        ...
        if not self.put_handler.done:
            self.put_handler.process()
            return
        if len(self.write_list) == 0:
            # all done
            self.put_finished(self.write_file_size)
            logger.info("no more writes to write, terminating")
            self.terminate_session()
            return
    ...
    def handle_write_reply(self, op, m):
        self.put_handler.on_reply(op, m)
        """handle OP_WriteFile reply"""
        if self.fh is None:
            logger.info("terminating session3")
            self.terminate_session()
            return
        if op.opcode != OP_Ack:
            print("Write failed")
    ...
    def cmd_cancel(self):
        ...
        logger.info("terminating session5")
        self.terminate_session()
    ...
    def mavlink_packet(self, m):
        ...
        if mtype == "FILE_TRANSFER_PROTOCOL":
            ...
            if op.opcode == OP_Nack:
                code = op.payload[0]
                logger.error(f"FTP_ERROR: {ftp_errors[code]}")
            ...
            if op.req_opcode == OP_ListDirectory:
            ...
            elif op.req_opcode == OP_OpenFileWO:
                self.handle_open_WO_reply(op, m)
                ...
                logger.error("FTP Unknown %s" % str(op))
...
class FTPUser(FTPModule):
    ...
    def request(self, cmd, params, timeout=0.5, max_retries=5, msg_timeout=0.3):
    ...
    def open_wo(self, path):
        self.put_handler.start()
        logger.info(f"opening {path} write-only")
        session = self.request("open_wo", path)
        logger.info(f"session = {session}")
        return session
    
    def write_file(self, file, offset, data, fh):
        return self.request("put2", (file, offset, data, fh))
...
if __name__ == "__main__":
    ...
    parser_list.add_argument("path", help="the path to list from", nargs="?", default="/")
    parser_list.set_defaults(func=lambda ftp_user, args: print(ftp_user.list(args.path)))
    ...
    parser_get.set_defaults(func=lambda ftp_user, args: ftp_user.get(args.filepath, args.save_as))
    ...
    parser_params.add_argument("save_as", help="the filename to save to", nargs="?", default="param.pck")
    parser_params.set_defaults(func=lambda ftp_user, args: ftp_user.params(args.filepath, args.save_as))
    ...
    for attr in dir(FTPUser):
        if attr.startswith("cmd") and not ("list" in attr or "get" in attr):
            cmd = attr[4:]
            extra_parser = subparsers.add_parser(cmd, help=getattr(FTPUser, attr).__doc__)
